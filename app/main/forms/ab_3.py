...
class FormAB3(FlaskForm):
    addr = StringField(lazy_gettext(u'3AB_addr'), validators=[DataRequired(message=lazy_gettext(u'Required'))])
    ...
    zip = StringField(lazy_gettext(u'3_zip'), validators=[DataRequired(message=lazy_gettext(u'Required')), Regexp(r"^\d{5}(-\d{4})?$", message=lazy_gettext(u'3_zip_help'))])
