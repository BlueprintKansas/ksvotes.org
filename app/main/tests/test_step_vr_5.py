...
def test_vr_5_no_id_provided(app, db_session, client):
    """
    ...
    form_payload = {}
    response = client.post('/vr/identification', data=form_payload, follow_redirects=False)
    assert response.status_code != 302

def test_valid_vr_5_returns_redirect(app,db_session,client):
    registrant = create_registrant(db_session)
    with client.session_transaction() as http_session:
        http_session['session_id'] = str(registrant.session_id)
