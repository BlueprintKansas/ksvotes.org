...
def test_vr_6_no_signature_provided(app, db_session, client):
    ...
    form_payload = {}
    response = client.post('/vr/preview', data=form_payload, follow_redirects=False)
    assert response.status_code != 302

def test_vr_6_bad_signature_provided(app, db_session, client):
    registrant = create_registrant(db_session)
    with client.session_transaction() as http_session:
        http_session['session_id'] = str(registrant.session_id)
...
def test_valid_vr_6_returns_redirect(app,db_session,client):
    registrant = create_registrant(db_session)
    with client.session_transaction() as http_session:
        http_session['session_id'] = str(registrant.session_id)
    ...
    assert response.status_code == 302
    assert ('/vr/affirmation' in redirect_data) == True
